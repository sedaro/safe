            ...
            if let Ok(message) = stream.read().await {
                match message {
                    AutonomyModeMessage::Active { nonce: new_nonce } => {
                        ...
                        let result = self
                            .simulator
                            .run(30.0 * 60.0 / 86_400.0, &results_path, None)
                            .await;
                        match &result {
                            Ok(output) => {
                        ...
                        stream
                            .write(RouterMessage::Command {
                                data: TimedCommand::Scheduled {
                                    cmd: Command::IridiumTransmitMsg(
                                        "Hello from SAFE!".to_string(),
                                    ),
...
fn min_contact_angle_to_iridium(frame: &TRD) -> f64 {
    ...
    for id in iridium_ids {
        let curr_angle = frame
            .get_by_field(format!("{}.contact_angle", id).as_str())
            .unwrap()
            .data
            .as_f64()
            .unwrap();
