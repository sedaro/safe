    ...
    C: Clone + Serialize + for<'de> Deserialize<'de> + Send + 'static,
{
    ...
    async fn run(
        &mut self,
        stream: Box<dyn Stream<AutonomyModeMessage<T>, RouterMessage<C>>>,
    ) -> Result<()>;
}
        ...
        if let Some(latest_telem) = telem_buffer.front() {
            ...
            match map.get(var).cloned() {
                Some(serde_json::Value::Number(n)) => {
                    n.as_f64().map(|v| Variable::Float64(Value::Literal(v)))
                }
...
pub struct Router<T, C, State = Build> {
    ...
    autonomy_mode_write_handles:
        Arc<tokio::sync::Mutex<HashMap<String, Box<dyn WriteHalf<AutonomyModeMessage<T>>>>>>,
    autonomy_mode_read_handles:
        Arc<tokio::sync::Mutex<HashMap<String, Box<dyn ReadHalf<RouterMessage<C>>>>>>,
    resolver: Resolver<T>,
}
    ...
    C: Clone + Serialize + for<'de> Deserialize<'de> + Send + 'static + std::marker::Sync,
{
    pub fn new(
        ...
        let autonomy_modes_transport: MpscTransport<RouterMessage<C>, AutonomyModeMessage<T>> =
            MpscTransport::new(config.router.command_channel_buffer_size); // TODO: Make transport configurable
        Self {
    ...
    async fn update_active_mode(&mut self) {
        ...
        if let Some((current_mode_name, _)) = &self.selected_mode {
            ...
            if let Activation::Hysteretic { enter: _, exit } = &current_mode.activation {
                ...
                match exit.eval(&self.resolver) {
                    ...
                    Err(e) => {
                        warn!(
                            "Failed to evaluate exit criteria for mode {}: {:?}.  Exiting.",
                            current_mode_name, e
                        ); // TODO: Figure out what desired behavior is here (and make configurable)
                        return;
                    }
                }
            }
        ...
        if !candidate_modes.is_empty() {
            ...
            if self.selected_mode.is_none()
                || highest_priority_mode.clone() != *self.selected_mode.clone().unwrap().0
            {
                ...
                if let Some((current_mode_name, _)) = &self.selected_mode {
                    let mode = self
                        .autonomy_modes
                        .get_mut(current_mode_name)
                        .expect("Mode not found");
                    mode.active = false;
                    let write_half = locked_write_handles.get_mut(current_mode_name).unwrap();
                    write_half
                        .write(AutonomyModeMessage::Inactive)
                        .await
                        .expect("Failed to send deactivate message to mode");
                }
                ...
                let mode = self
                    .autonomy_modes
                    .get_mut(highest_priority_mode)
                    .expect("Mode not found");
                mode.active = true;
                let write_half = locked_write_handles.get_mut(highest_priority_mode).unwrap();
                write_half
                    .write(AutonomyModeMessage::Active { nonce })
                    .await
                    .expect("Failed to send activate message to mode");
    ...
    C: Clone + Serialize + for<'de> Deserialize<'de> + Send + 'static + std::marker::Sync,
{
    ...
    pub async fn register_autonomy_mode<M: AutonomyMode<T, C>>(
        &mut self,
        mut mode: M,
    ) -> Result<()> {
        ...
        let handle = tokio::spawn(
            async move {
                ...
                if let Err(e) = mode.run(client_stream).await {
        ...
        self.autonomy_modes.insert(mode_name.clone(), managed_mode);
        let (read_half, write_half) = server_stream.split();
        self.autonomy_mode_write_handles
            .lock()
            .await
            .insert(mode_name.clone(), write_half);
        self.autonomy_mode_read_handles
            .lock()
            .await
            .insert(mode_name.clone(), read_half);
...
mod tests {
    ...
    fn test_eval() {
        ...
        assert_eq!(
            Expr::Equal(
                Box::new(Expr::Term(Variable::Float64(Value::Literal(1.5)))),
        ...
        assert_eq!(
            Expr::Equal(
                Box::new(Expr::Term(Variable::Bool(Value::Literal(false)))),
                Box::new(Expr::Term(Variable::Bool(Value::AverageTelemetryRef {
                    name: "in_sunlight".to_string(),
                    points: 5,
                }))),
