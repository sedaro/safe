...
impl Flight {
    pub async fn new() -> Self {
        ...
        let c2_to_router_transport: MpscTransport<Telemetry, Command> = MpscTransport::new(config.router.telem_channel_buffer_size);
    ...
    pub fn register_autonomy_mode<M: AutonomyMode>(&mut self, mode: M) { // TODO: Implement dynamic registration
            ...
            loop {
                match client_transport.accept().await {
                    Ok(stream) => {
                        ...
                        tokio::spawn(async move {
                            handle_c2_connection(stream, c2_telem_stream).await;
                        });
                    }
...
async fn handle_c2_connection(
    ...
    if let Ok(msg) = stream.read().await {
        ...
        } else if let Ok(tlm) = serde_json::from_str::<Telemetry>(&msg) {
