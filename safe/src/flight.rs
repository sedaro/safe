...
pub struct Flight<T, C, State = Build> {
    ...
    tracing_layers: Vec<
        tracing_subscriber::fmt::Layer<
            Registry,
            JsonFields,
            tracing_subscriber::fmt::format::Format<Json>,
            NonBlocking,
        >,
    >,
    ...
    C: Clone + Serialize + for<'de> Deserialize<'de> + Send + 'static + Sync,
{
    pub async fn new() -> Self {
        let config = Figment::new()
            ...
            .unwrap_or_else(|e| {
                panic!(
                    "Failed to load user-defined configuration: {}.  Continuing.",
                    e
                );
            });
        ...
        let c2_to_router_transport: MpscTransport<T, C> =
            MpscTransport::new(config.router.telem_channel_buffer_size);
        let c2_to_router_transport_handle = c2_to_router_transport.handle();
        let router = Router::new(
            // TODO: Consider just making this part of Flight
    ...
    pub fn client_to_c2_transport(
        mut self,
        transport: impl Transport<String, String> + 'static,
    ) -> Self {
    ...
    C: Clone + Serialize + for<'de> Deserialize<'de> + Send + 'static + Sync,
{
    ...
    pub async fn register_autonomy_mode<M: AutonomyMode<T, C>>(&mut self, mode: M) -> Result<()> {
        ...
        let (non_blocking, guard) = tracing_appender::non_blocking(
            tracing_appender::rolling::daily("./logs", format!("{}.log", mode.name().clone())),
        );
    ...
    C: Clone + Serialize + for<'de> Deserialize<'de> + Send + 'static + Sync,
{
    ...
    async fn handle_c2_connection(
        ...
        match stream.read().await {
            Ok(msg) => {
                if let Ok(logs_request) = serde_json::from_str::<LogsRequest>(&msg) {
                    // TODO: Rewrite this once tested and more tightly couple to observability system
