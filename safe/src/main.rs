...
async fn main() -> Result<()> {
    let mut flight: Flight<Telemetry, TimedCommand> = Flight::new().await.client_to_c2_transport(
        TcpTransport::new("127.0.0.1", 8001)
            .await
            .unwrap_or_else(|e| {
    ...
    let mode = AttitudeControlAnomalyRecovery::new(
        ...
        100,
        12,
        SedaroSimulator::new(&std::path::PathBuf::from("./simulators/imaging"))
            .venv(std::path::PathBuf::from(env::var("SAFE_VENV").unwrap()))
    ...
    let mode = ContactAnalysis::new(
        ...
        5,
        Activation::Immediate(Expr::not(Expr::Term(Variable::Bool(Value::TelemetryRef(
            "in_sunlight".to_string(),
        ))))),
        // Activation::Immediate(Expr::Term(Variable::Bool(Value::Literal(true)))),
        SedaroSimulator::new(&std::path::PathBuf::from("./simulators/iridium"))
            .venv(std::path::PathBuf::from(env::var("SAFE_VENV").unwrap()))
            .timeout(Duration::from_secs_f64(20.0)),
    );
    flight.register_autonomy_mode(mode).await?;
    ...
    let (non_blocking_a, _guard_a) = tracing_appender::non_blocking(
        tracing_appender::rolling::daily("./logs", "AttitudeControlAnomalyRecovery.log"),
    );
    let (non_blocking_b, _guard_b) = tracing_appender::non_blocking(
        tracing_appender::rolling::daily("./logs", "IridiumContactAnalysis.log"),
    );
    let (non_blocking_c, _guard_c) = tracing_appender::non_blocking(
        tracing_appender::rolling::daily("./logs", "MultiObjectiveConOpsOptimization.log"),
    );
        ...
        .with(
            tracing_subscriber::fmt::layer()
                ...
                .with_filter(EnvFilter::new(
                    "[{autonomy_mode=AttitudeControlAnomalyRecovery}]=trace",
                )),
        )
        .with(
            tracing_subscriber::fmt::layer()
                ...
                .json()
                .with_filter(EnvFilter::new(
                    "[{autonomy_mode=IridiumContactAnalysis}]=trace",
                )),
        )
        .with(
            tracing_subscriber::fmt::layer()
                ...
                .json()
                .with_filter(EnvFilter::new(
                    "[{autonomy_mode=MultiObjectiveConOpsOptimization}]=trace",
                )),
        )
        .init();
...
mod tests {
    use crate::{
        c2::{AutonomyModeMessage, RouterMessage},
        definitions::{Activation, Expr, Value, Variable},
        flight::Flight,
        router::AutonomyMode,
        transports::{Stream, TestTransport, Transport},
    };
    use anyhow::Result;
    ...
    impl TestAutonomyMode {
        fn new(name: &str, priority: u8) -> Self {
            Self {
                name: name.to_string(),
                priority,
                activation: Activation::Immediate(Expr::Term(Variable::Bool(Value::Literal(true)))),
            }
        }
    ...
    impl AutonomyMode<TestTelemetry, TestCommand> for TestAutonomyMode {
        ...
        async fn run(
            &mut self,
            mut stream: Box<
                dyn Stream<AutonomyModeMessage<TestTelemetry>, RouterMessage<TestCommand>>,
            >,
        ) -> Result<()> {
    ...
    async fn test_flight_basic() {
        ...
        let mode = TestAutonomyMode::new("Mode B", 1).with_activation(Activation::Hysteretic {
            enter: Expr::not(Expr::greater_than(
        ...
        client_rx_stream
            .write(serde_json::to_string(&CommandsRequest {}).unwrap())
            .await
            .unwrap();

        let mut client_tx_stream = client_transport_handle.connect().await.unwrap();
        client_tx_stream
            .write(serde_json::to_string(&TestTelemetry { value: 200.0 }).unwrap())
            .await
            .unwrap();
        assert_eq!(
            serde_json::from_str::<TestCommand>(client_rx_stream.read().await.unwrap().as_str())
                .unwrap()
                .value,
            "Mode A received 200".to_string()
        );
        let mut client_tx_stream = client_transport_handle.connect().await.unwrap();
        client_tx_stream
            .write(serde_json::to_string(&TestTelemetry { value: 99.0 }).unwrap())
            .await
            .unwrap();
        assert_eq!(
            serde_json::from_str::<TestCommand>(client_rx_stream.read().await.unwrap().as_str())
                .unwrap()
                .value,
            "Mode B received 99".to_string()
        );
        let mut client_tx_stream = client_transport_handle.connect().await.unwrap();
        client_tx_stream
            .write(serde_json::to_string(&TestTelemetry { value: 125.0 }).unwrap())
            .await
            .unwrap();
        assert_eq!(
            serde_json::from_str::<TestCommand>(client_rx_stream.read().await.unwrap().as_str())
